...
- **Áudio é entrada de primeira classe.** O usuário pode explicar o negócio e os briefings **falando** (o sistema transcreve). Menos digitação, mais contexto.
...
- Estrutura no padrão **ABO**: divide a verba entre **campanhas por produto** e cria **conjuntos** para cada anúncio.
- Sugere **criativos em vídeo e imagem** para cada anúncio.
- Cada criativo carrega o `content_id` no `utm_content` → fecha o loop de atribuição com o CRM (leads → matrículas).

> O Módulo Anúncios é uma camada de **planejamento de mídia** (verba, campanhas, conjuntos) acima do motor de criativos. É um bloco novo e substancial — merece sua própria fase.
...
2. **Onboarding por áudio** — transcrição (Whisper/Deepgram) + entrevista dirigida que monta o contexto rico.
...
**Em aberto (preciso de você) ❓:**
1. **Onboarding:** a lista de contexto da §3 está completa/assertiva? O que cortar/adicionar?
2. **Mix orgânico:** existe um padrão sugerido (ex.: 60% marca / 40% produto) ou sempre manual?
3. **Papéis:** na revenda, o cliente opera sozinho ou você opera pra ele? (afeta o painel)
4. **Anúncios — escopo:** o sistema só **sugere** a estrutura ABO/verba (você monta no Gerenciador do Meta), ou a ideia futura é **publicar via API do Meta**?
5. **Transcrição:** preferência de serviço pro áudio (Whisper/OpenAI, Deepgram, Groq…) — pt-BR.
